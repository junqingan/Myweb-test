...
!
    function(a) {
        ...
        a.laydate = function(b) {
        ...
        }, c.canChooseDate=function(){
            var arr = c.options.arr?c.options.arr:b.arr;
            if(arr.length){
                $('#laydate_table').find('td').addClass('laydate_void');
                console.log(arr.length);
                for(var i=0;i<arr.length;i++){
                    var dateArr=arr[i].split('-');
                    var dateArrY=dateArr[0];
                    var dateArrM=dateArr[1]<10?dateArr[1].substring(1):dateArr[1];
                    var dateArrD=dateArr[2]<10?dateArr[2].substring(1):dateArr[2];

                    $('#laydate_table').find('td[y='+dateArrY+'][m='+dateArrM+'][d='+dateArrD+']').removeClass().addClass('canchoose');
                }
            }
        }, c.trim = function(a) {
            ...
            c.each(new Array(14), function(b) {
                ...
                "y" === c.checkVoid(b.getAttribute("y"))[0] ? c.addClass(b, h[1]) : c.on(b, "click", function(a) {
                    c.stopmp(a).reshow(), c.viewDate(0 | this.getAttribute("y"), c.ymd[1], c.ymd[2]),c.canChooseDate(b)
                })
            })
        }, c.initDate = function() {
            ...
            c.addClass(d.body, "laydate_body"), h.tds = b("#laydate_table td"), h.mms = b("#laydate_ms span"), h.year = b("#laydate_y"), h.month = b("#laydate_m"), c.each(b(e.box + " .laydate_ym"), function(a, b) {
                ...
                0 === a ? c.ymd[0]-- : 1 === a ? c.ymd[0]++ : 2 === a ? e.YY -= 14 : e.YY += 14, 2 > a ? (c.viewDate(c.ymd[0], c.ymd[1], c.ymd[2]), c.reshow()) : c.viewYears(e.YY),c.canChooseDate(b)
            }, c.each(b("#laydate_YY .laydate_tab"), function(a, b) {
                c.on(b, "click", function(b) {
                    c.stopmp(b), e.tabYear(a)
                })
            }), e.tabMonth = function(a) {
                a ? (c.ymd[1]++, 12 === c.ymd[1] && (c.ymd[0]++, c.ymd[1] = 0)) : (c.ymd[1]--, -1 === c.ymd[1] && (c.ymd[0]--, c.ymd[1] = 11)), c.viewDate(c.ymd[0], c.ymd[1], c.ymd[2]), c.canChooseDate(b)
                ...
                c.on(b, "click", function(a) {
                    c.stopmp(a).reshow(), c.hasClass(this, h[1]) || c.viewDate(c.ymd[0], 0 | this.getAttribute("m"), c.ymd[2]),c.canChooseDate(b)
                })
